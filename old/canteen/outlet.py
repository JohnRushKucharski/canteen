...
class Outlets:
    ...
    sorter: None|Callable[[list[Outlet]], tuple[Outlet,...]] = None

    def __post_init__(self) -> None:
        self._index = 0  # Initialize iteration index
        if self.sorter:
            self.outlets = self.sorter(self.outlets)
        else:
            sorted_outlets = sorted(self.outlets, key=lambda o: (o.location, o.name), reverse=True)
            self.outlets = tuple(sorted_outlets)  # Outlets sorted by location, name
