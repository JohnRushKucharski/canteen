...
# '''
# from typing import Callable, Self, no_type_check
...
#     k: float = 1.0
#     '''
#     Parameter controlling shape of depreciation schedule.
...
#     = 1.0 is no acceleration (normal depreciation),
#     > 1.0 if faster than linear acceleration.
#     < 1.0 is invalid.

#     Deferred maintenance can speed up deterioration (i.e., depreciation).
#     '''
...
#             '''
#             maintenance = min(maintenance, self.parameters.maintenance_requirement)
#             recap = max(0.0, maintenance - self.parameters.maintenance_requirement)

#             t = min(self.parameters.n,
#                     self.parameters.inverse_ft(self.portion_remaining) + self.parameters.scheduler(maintenance)) # pylint: disable=line-too-long
...
#         return self.depreciation_fn(maintenance) # type: ignore
